...
class findFaceGetPulse(object):
    def __init__(
        ...
        dpath = resource_path(config.classifierPath)
    ...
    def run(self, cam):
        ...
        if self.find_faces:
            ...
            x, y, w, h = self.face_rect
            cv2.putText(
                self.frame_out, "Face", (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, col
            )
            self.draw_rect(forehead1)
            x, y, w, h = forehead1
            cv2.putText(
                self.frame_out, "Forehead", (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, col
            )
            return
